...
class ConversionResult:
    ...
    - markdown: 转换后的 markdown 正文（不含 FILE START/END 分隔符）。
                失败/跳过时也要给出占位文本。
                注意：主流程会在调用 build_file_block 写 spool 后把这里置空
                以释放内存。所以 splitter 之后访问 markdown 是不安全的。
    - warnings: 可选的警告列表（例如图片型 PDF、行数被截断等）
    - error: 失败原因摘要
    - skip_reason: 跳过原因（如 "OCR 未启用"、"不支持的扩展名"）
    - extra: 类型特定的附加字段（OCR 字符数、耗时、错误码等），用于报告
    - block_spool_path: 主流程写到 spool/ 的"完整 file_block"文件路径。
                       splitter 流式读取它,避免所有文件 block 同时驻留内存。
                       None 表示尚未生成 spool 或不需要(罕见,只在写入失败时发生)。
    - block_spool_bytes: spool 文件大小(便于 splitter 不开文件就能规划字节)。
    """
    ...
    block_spool_path: Path | None = None
    block_spool_bytes: int = 0
